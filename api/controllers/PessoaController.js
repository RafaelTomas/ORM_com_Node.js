...
class PessoaController {
    ...
    static async atualizaPessoa(req, res) {
        ...
        try {
            await pessoasServices.update(novasInfo, { where: { id: Number(id)}})
            const pessoaAtualizada = await pessoasServices.findOne( { where: { id: Number(id)}})
    ...
    static async pegaMatricula(req, res){
        const { estudanteId } = req.params
        try {
            const pessoa = await pessoasServices.findOne({ where: { id: Number(estudanteId)}})
    ...
    static async cancelaPessoa(req, res){
        ...
        try {
            ...
            return res.status(200).json({ message: `Matricula referente ao estudante '${estudanteId}' cancelada`})
